...
public class NCShell {
	...
	private String[] readChatCommandFromStdIn(NCConnector ncclient) {
		...
		while (true) {
			...
			case NCCommands.COM_SEND:
				// Leemos todo el mensaje de la terminal
			...
			case NCCommands.COM_PRIVATE: {
				...
				// Leemos el mensaje que se quiere Enviar
				while (st.hasMoreTokens())
					mensaje.append(st.nextToken() + " ");
				vargs.add(mensaje.toString());
				break;
			}
			default:
	...
	private boolean validateCommandArguments(String[] args) {
		...
		case NCCommands.COM_SEND:
			...
			break;
		// private requiere el parámetro <message> y el <dest>
		case NCCommands.COM_PRIVATE:
			if (args.length == 0 || args.length == 1) {
				System.out.println("Correct use: private <dst> <message>");
				return false;
			}
			break;
		default:
